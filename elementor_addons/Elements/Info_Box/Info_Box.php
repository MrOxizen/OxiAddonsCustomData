...
class Info_Box extends Widget_Base {
	...
	protected function _register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'default' => esc_html__( 'This is an icon box', SA_ELEMENTOR_TEXTDOMAIN )
			]
		);
		$this->add_control(
	...
	protected function sa_el_infobox_after($settings) {
		ob_start();?></div><?php
		if( 'yes' == $settings['sa_el_show_infobox_clickable'] ) : ?></a><?php endif;
		echo ob_get_clean();
	}
	...
	protected function render_infobox_icon($settings) {
		...
		if( 'none' == $settings['sa_el_infobox_img_or_icon'] ) return;
		...
		?>
			<div <?php echo $this->get_render_attribute_string('infobox_content'); ?>>
				...
				<?php if( 'yes' == $settings['sa_el_show_infobox_content'] ) : ?>
					...
					<?php elseif( 'template' === $settings['sa_el_infobox_text_type'] ) :
	...
	protected function render_infobox_button( $settings ) {
		if('yes' == $settings['sa_el_show_infobox_clickable'] || 'yes' != $settings['sa_el_show_infobox_button']) return;
